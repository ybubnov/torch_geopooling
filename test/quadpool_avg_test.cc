...
BOOST_AUTO_TEST_CASE(avg_quad_pool2d_training)
{
    ...
    auto [tiles_out, weight_out] = avg_quad_pool2d(tiles, input_train, weight, exterior, true);
    ...
    std::tie(tiles_out, weight_out) = avg_quad_pool2d(tiles_out, input_test, weight, exterior, true);

    BOOST_REQUIRE_EQUAL(weight_out.sizes(), torch::IntArrayRef({1, weight.size(1)}));

    auto weight_mean = at::mean(weight.index({Slice(8, 12), Slice()}), 0);
...
BOOST_AUTO_TEST_CASE(avg_quad_pool2d_backward_grad)
{
    ...
    auto tiles = torch::tensor({
        ...
        {1, 1, 0},
        {2, 0, 1}, // (1,0,0) -> weight[1]
        {2, 1, 0}, // (1,0,0) -> weight[2]
        {2, 2, 0}, // (1,1,0) -> weight[3]
        {2, 2, 1}, // (1,1,0) -> weight[4]
    }, tiles_options);

    auto tensor_options = torch::TensorOptions().dtype(torch::kFloat64);
    auto weight = torch::rand({5, 3}, tensor_options.requires_grad(true));

    auto input = torch::tensor({
        {0.1, 0.1}, // (2,0,0) -> avg(weight[0], weight[1], weight[2])
        {0.2, 0.1}, // (2,0,0) -> avg(weight[0], weight[1], weight[2])
        {1.3, 0.2}  // (2,2,0) -> avg(weight[3], weight[4])
    }, tensor_options);
    ...
    BOOST_REQUIRE_EQUAL(grad_weight.sizes(), torch::IntArrayRef({5, 3}));
